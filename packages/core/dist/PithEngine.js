export class PithEngine {
    ...
    scoreWord(word, freq, totalWords, isFirstInLine, isSentenceStart = false, isQuestion = false) {
        ...
        if (/[^a-zA-ZÀ-ÿ\s.,;:!?'"/-]/.test(word))
            return 100; // Allow hyphens for hyphenated tech words
        ...
        if (clean.length === 3) {
            if (!/[aeiouà-ú]/i.test(clean))
                score += 3; // Acronyms/Tech terms (e.g. jwt, npm, sql)
            if (isQuestion)
                score += 2; // In questions, short words like 'how', 'que', 'why' matter more
            if (/^(bom|mal|bad|boa|bug|api|app|web)$/i.test(clean))
                score += 5; // Vital short nouns/adjectives
        }
        ...
        if (clean.length >= 6 && PithEngine.VERB_ENDING.test(clean.toLowerCase()))
            score -= 3;
    ...
    queryPipeline(text) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (PithEngine.INTENT_TAGS.has(clean.toLowerCase()))
                continue;
    ...
    preserveLayer(text) {
        const map = new Map();
        let counter = 0;
        const ph = (v) => { const k = `\u0000P${counter++}\u0000`; map.set(k, v); return k; };
        let r = text;
    ...
    patternLayer(text) {
        ...
        r = r.replace(/\b(\w{2,}(?:\/\w{2,}){1,})\b/g, (_m, group) => {
            ...
            const collapsed = parts.map((p) => p.length > 4 ? p[0].toUpperCase() + p.slice(1, 3).toLowerCase() : p).join('|');
            return '[' + collapsed + ']';
        });
...
PithEngine.INTENT_TAGS = new Map([
    // PT
...
// Scoring thresholds
PithEngine.QUERY_THRESHOLD = 5;
PithEngine.COMPRESS_THRESHOLD = 4;
// Morphological patterns (algorithmic, not word lists)
PithEngine.ADJECTIVE_SUFFIX = /(?:ário|ária|oso|osa|ivo|iva|ável|ível|inho|inha|ante|ente|udo|uda|ário|ária|ary|ous|ive|able|ible|ful|less|ical|ial)$/i;
PithEngine.VERB_ENDING = /(?:[aei]r|[aei]ndo|[aei]ram|[aei]va[ms]?|[aei]rá|[aei]rão|[aei]sse[ms]?)$/i;
