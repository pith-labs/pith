...
function isInfinitiveCandidate(word) {
  ...
  return word.length >= 3 && word.length <= 24 && !word.startsWith("~") && !/\d/.test(word) && !/^[A-Z]/.test(word) && !ADJECTIVE_SUFFIX.test(lower) && VERB_INFINITIVE.test(lower) && isRomanceInfinitiveShape(lower);
}
...
function weightInfinitiveAction(baseScore, word, freq, totalWords) {
...
var PithEngine = class {
  ...
  detectMode(text) {
    ...
    if (hasQuestion && nonEmptyLines <= 3) {
