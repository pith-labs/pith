...
function slugToken(raw) {
  return raw.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 32);
}
function detectLanguageHint(text) {
  if (/\b(que|como|para|com|não|ção|ções)\b/i.test(text))
    return "pt";
  if (/\b(the|and|with|for|how|please)\b/i.test(text))
    return "en";
  if (/\b(que|como|para|con|por|ción|ciones)\b/i.test(text))
    return "es";
  if (/\b(comment|pour|avec|sans|tion|tions)\b/i.test(text))
    return "fr";
  return "unknown";
}
function detectOutputFormat(text) {
  if (/\bjson\b/i.test(text))
    return "json";
  if (/```|\btypescript\b|\bjavascript\b|\bpython\b|\bcode\b/i.test(text))
    return "code";
...
function scoreDomains(text) {
  ...
  for (const [rx, domain] of DOMAIN_PATTERNS) {
    if (!rx.test(text))
      continue;
    let score = 5;
    if (domain === "async-processing" && /\b(retry|dlq|idempot|worker)\b/i.test(text))
      score += 4;
    if (domain === "backend" && /\b(api|endpoint|route|http)\b/i.test(text))
      score += 3;
    if (domain === "llm" && /\b(prompt|tokens?|openai|claude|context)\b/i.test(text))
      score += 3;
...
function pickSlots(text) {
  ...
  for (const key of Object.keys(SLOT_PATTERNS)) {
...
function parseIntentIR(text) {
  const trimmed = text.trim();
  const action = pickAction(trimmed);
  const domainScores = scoreDomains(trimmed);
  const domain = domainScores.map((d) => d.name);
  const signalScore = (/\?/.test(trimmed) ? 1 : 0) + (domainScores.length ? 1 : 0) + (pickEntities(trimmed).length ? 1 : 0) + (pickMustInclude(trimmed).length || pickMustAvoid(trimmed).length ? 1 : 0);
...
function generateMachinePrompt(ir) {
  const parts = [];
  parts.push(`act=${ir.intent.action}`);
  if (ir.intent.domain.length)
    parts.push(`dom=${ir.intent.domain.join(",")}`);
  if (ir.intent.entities.length)
    parts.push(`ent=${ir.intent.entities.join(",")}`);
  parts.push(`conf=${ir.intent.confidence.toFixed(2)}`);
  parts.push(`fmt=${ir.constraints.outputFormat}`);
  if (typeof ir.constraints.maxLength === "number")
    parts.push(`max=${ir.constraints.maxLength}`);
  if (ir.constraints.mustInclude.length)
    parts.push(`must+${ir.constraints.mustInclude.join(",")}`);
  if (ir.constraints.mustAvoid.length)
    parts.push(`must-${ir.constraints.mustAvoid.join(",")}`);
  if (ir.constraints.preserveNegation)
    parts.push("keep-negation");
  if (ir.signals.hasCode)
    parts.push("preserve-code");
  return parts.join(" | ");
}

// ../core/src/domainWeights.ts
var DOMAIN_WEIGHTS_V1 = {
  version: "v1",
  focus: {
    backend: {
      idempotency: 10,
...
function rankByWeights(values, weights) {
  return Array.from(new Set(values)).filter(Boolean).sort((a, b) => (weights[b] ?? 0) - (weights[a] ?? 0) || a.localeCompare(b));
}

// ../core/src/domainPlanner.ts
function uniq(xs) {
  return Array.from(new Set(xs)).filter(Boolean);
}
function pickTrack(ir) {
  const top = ir.intent.domain[0] ?? "";
  if (top === "backend")
    return "backend";
  if (top === "async-processing")
    return "worker";
  if (top === "llm")
    return "llm";
  if (top === "testing")
...
function buildDomainPlan(ir) {
  const track = pickTrack(ir);
  if (track === "backend")
    return planBackend(ir);
  if (track === "worker")
    return planWorker(ir);
  if (track === "llm")
    return planLLM(ir);
  if (track === "testing")
    return planTesting(ir);
  if (track === "data")
    return planData(ir);
  return planGeneric(ir);
}
function serializeDomainPlan(plan) {
  const prefix = {
    backend: "be",
    worker: "wk",
    llm: "lm",
...
// ../core/src/irOpcode.ts
function pickModeFromIR(ir) {
  if (ir.signals.hasQuestion)
    return "Q";
  if (ir.signals.hasCode)
    return "C";
  return "Q";
}
function generateOpcodeFromIR(ir, originalText, ultraCompact = true) {
  const mode = pickModeFromIR(ir);
  const domains = ir.intent.domain.map((d) => `#${d}`);
  const entities = ir.intent.entities.map((e) => `@${e}`);
  const attrs = [];
  if (typeof ir.constraints.maxLength === "number")
    attrs.push(`?max${ir.constraints.maxLength}`);
  for (const q of ir.slots.quality)
    attrs.push(`?${q}`);
  const payloadParts = [];
  const plan = buildDomainPlan(ir);
  payloadParts.push(`pl:${serializeDomainPlan(plan)}`);
  if (ir.slots.runtime.length)
    payloadParts.push(`rt:${ir.slots.runtime.join(",")}`);
  if (ir.slots.transport.length)
    payloadParts.push(`tr:${ir.slots.transport.join(",")}`);
  if (ir.slots.storage.length)
    payloadParts.push(`db:${ir.slots.storage.join(",")}`);
  if (ir.constraints.mustInclude.length)
    payloadParts.push(`in:${ir.constraints.mustInclude.join(",")}`);
  if (ir.constraints.mustAvoid.length)
    payloadParts.push(`out:${ir.constraints.mustAvoid.join(",")}`);
...
var PithEngine = class {
  ...
  optimize(text, options = { ultraCompact: true }) {
    try {
      ...
      const result = pre.mode === "compress" ? compressPipeline(pre.text, pre.options) : pre.mode === "conversational" ? conversationalPipeline(pre.text, pre.options) : queryPipeline(pre.text, pre.options);
  ...
  runPluginPre(input) {
    return this.plugins.reduce((acc, p) => {
      const hook = p.hooks?.beforeOptimize;
      if (!hook)
        return acc;
      return hook(acc) ?? acc;
    }, input);
  ...
  runPluginPost(input) {
    return this.plugins.reduce((acc, p) => {
      const hook = p.hooks?.afterOptimize;
      if (!hook)
        return acc;
